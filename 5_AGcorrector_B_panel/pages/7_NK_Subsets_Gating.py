...
#!/usr/bin/env python3
"""
Title:7_NK_Subsets_Gating.py
Date: 2025-05-27
Author: Wenxia Ren
Description:
Step1: Correcting the NK subsets gating in B panel.
"""
...
def gateGeneralDataSet(fcs, y1,sigma_n,hixlim_step,y2):
    SSC="SSC 488/10-A"
    FSC="FSC 488/10-A"
    IgA="FITC IgA-A"
    ...
    #### Correct the ylim_1
    # y1 is the percentage, the default is 1
    ylim_1 =  ylim_1 *y1
    ...
    # the second parameter: sigma_n, the default is 3
    bilogXlim = mean- sigma*sigma_n
    hixlim = ag.inverseTransformWrapper(vX=[bilogXlim], scale='bilog', T=100)[0]
    # the third parameter, the default of it is 0 
    hixlim = hixlim + hixlim_step
    # CD56+CD16+ NK
    fileName1="/home/wenxiaren/AGcorrector_B_panel/tmp"+date+"-"+plate+"-"+sampleName+"-doublePosNK.png"
    doublePosNK = ag.gateCorner(fcs, "CD56posCD16posNK", parentGate=CD14neg, xCol=CD16, yCol=CD56, xThresh=hixlim, yThresh=ylim_1, yOrientation='upper', xOrientation='upper', scale='bilog',T=100, filePlot=fileName1)
    fcs.update(ag.AGgate(doublePosNK,CD14neg,CD16,CD56,"doublePosNK"), QC=True, xlim=[-1000,120000], ylim=[-1000,120000], scale='bilog', T=100)
    # CD56-CD16+ NK
    fileName2="/home/wenxiaren/AGcorrector_B_panel/tmp"+date+"-"+plate+"-"+sampleName+"-CD56negCD16posNK.png"
    CD56negCD16posNK = ag.gateCorner(fcs, "CD56negCD16posNK", parentGate=CD14neg, xCol=CD16, yCol=CD56, xThresh=hixlim, yThresh=ylim_1, yOrientation='lower', xOrientation='upper', scale='bilog',T=100, filePlot=fileName2)
    fcs.update(ag.AGgate(CD56negCD16posNK,CD14neg,CD16,CD56,"CD56negCD16posNK"), QC=False, xlim=[-1000,120000], ylim=[-1000,120000])
    # CD56+CD16- NK
    ...
    # Correct the ylim-2, the default is 1
    ylim_2 = ylim_2 * y2
    fileName3="/home/wenxiaren/AGcorrector_B_panel/tmp"+date+"-"+plate+"-"+sampleName+"-CD16negCD56posNK.png"
    CD16negCD56posNK = ag.gateCorner(fcs, "CD16negCD56posNK", xCol=CD16, yCol=CD56, parentGate=CD14neg, xOrientation='lower',yOrientation='upper', xThresh=hixlim, yThresh=ylim_2, scale='bilog',T=100,filePlot=fileName3)
    fcs.update(ag.AGgate(CD16negCD56posNK,CD14neg,CD16,CD56,"CD16negCD56posNK"), QC=False, xlim=[-1000,120000], ylim=[-1000,120000])
    # CD56-CD16- NK
    fileName4="/home/wenxiaren/AGcorrector_B_panel/tmp"+date+"-"+plate+"-"+sampleName+"-doubleNegNK.png"
    doubleNegNK = ag.gateCorner(fcs, "doubleNegNK", xCol=CD16, yCol=CD56, parentGate=CD14neg, xOrientation='lower',yOrientation='lower', xThresh=hixlim, yThresh=ylim_2, scale='bilog',T=100,filePlot=fileName4)
    fcs.update(ag.AGgate(doubleNegNK,CD14neg,CD16,CD56,"doubleNegNK"), QC=False, xlim=[-1000,120000], ylim=[-1000,120000])  
...
with st.sidebar:
    ...
    y1 = st.number_input("y1", min_value=-1.0, max_value=15.0, value=1.0,format="%.1f")
    sigma_n = st.number_input("sigma_n", min_value=-10.0, max_value=15.0, value=3.0,format="%.1f")
    hixlim_step= st.number_input("hixlim_step", min_value=-100000, max_value=100000, value=0)
    y2 = st.number_input("y2", min_value=-1.0, max_value=15.0, value=1.0,format="%.1f")
    ...
    unsafe_allow_html=True
            ...
            if Update:
                if 'fcs' in st.session_state and st.session_state['fcs'] is not None:
                    ...
                    fileName_1, fileName_4, fileName_3,fileName_2 = gateGeneralDataSet(fcs, y1,sigma_n,hixlim_step,y2)
                    #set the layput of the updated images
                    fig_path = [fileName_1, fileName_4, fileName_3,fileName_2]
                    cols = st.columns(4)
                    clo_i = 0
                    for i  in fig_path:
                        with cols[clo_i]:
            ...
            if Confirm:
                if 'current_id' in st.session_state:
                    ...
                    para_dict = {
                        f"y1 {y1}": y1,
                        f"sigma_n {sigma_n}": sigma_n,
                        f"hixlim_step {hixlim_step}": hixlim_step,
                        f"y2 {y2}": y2,
                    }
                YAML_PATH = "/home/wenxiaren/AGcorrector_B_panel/tmp/NK_correction.yaml"
