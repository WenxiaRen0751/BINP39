...
#!/usr/bin/env python3
"""
Title:8_CD16+CD56-_NK_Gating.py
Date: 2025-05-27
Author: Wenxia Ren
Description: 
Step2: Correcting the NK subsets: CD16+CD56-_NK_Gating in B panel.
...
corrections_yaml_file = "/home/wenxiaren/B_panel_new_batch/old/correctionv2.yaml"
...
def gateGeneralDataSet(fcs, xThresh2, Thresh2, center, adjustAngle_2,widthScale_2,heightScale_2):
    SSC="SSC 488/10-A"
    FSC="FSC 488/10-A"
    IgA="FITC IgA-A"
    ...
    if sample_ids in correction_16 or sample_ids in correction_17:
        ...
        heightScale_1 = 0.5
    elif sample_ids in corrections_dict_raw['Singlet Gating']['heightScale_2']:
        heightScale_1 = 2
    else:
        heightScale_1 = 4 
    # Setting width
    ...
    CD45pos = ag.gateCorner(fcs, "CD45pos", xCol=CD3,yCol=CD45, parentGate=singlets,xThresh=xlim1,yThresh=ylim1,xOrientation='lower',yOrientation='upper',scale='bilog',T=500, filePlot=None)
    ...
    nonclassical_monocytes = ag.gateCorner(fcs, "NonClassicalMonocytes", xCol=CD14, yCol=CD16, parentGate=no_clutter_CD19neg, xThresh=xlim_hi, yThresh=ylim, xOrientation='upper',yOrientation='upper',scale='bilog', T=500,filePlot=None)  
    ...
    if xlim_lo < xlim_hi:
        ...
        ag.backGate(fcs,xCol=CD14, yCol=CD16,population=CD14neg,background_population=no_clutter_CD19neg,scale='bilog',T=500,filePlot=None, markersize=0.1)
        ...
        CD14neg = ag.gateThreshold(fcs, 'CD14neg', xCol=CD14, yCol=CD16, parentGate=no_clutter_CD19neg, thresh=xlim, population='lower', scale='bilog',T=500,filePlot=None)
    ...
    Intermediate_monocytes = ag.gateBox(fcs,'Intermediate_monocytes', xCol=CD14, yCol=CD16, xThreshRight=10000, yThreshTop=ylim, xThreshLeft=xlim_lo, yThreshBottom=CD16low, 
        Outer=False, parentGate=no_clutter_CD19neg, bins=300, scale='bilog',T=500, update=False,filePlot=None, QC=False)
    ...
    if sample_ids in corrections_dict_raw['NK Gating']['CUSTOM_y1=1000']:
        ...
        ylim_1 =  ylim_1 *0.5
    elif sample_ids in corrections_dict_raw['NK Gating']['y1=2.0']:
        ylim_1 =  ylim_1*2.0
    elif sample_ids in corrections_dict_raw['NK Gating']['y1=2.5']:
        ylim_1 =  ylim_1 *2.5
    elif sample_ids in corrections_dict_raw['NK Gating']['y1=3.0']:
        ylim_1 =  ylim_1 *3  
    elif sample_ids in corrections_dict_raw['NK Gating']['y1=4.0']:
        ylim_1 =  ylim_1 *4.0
    elif sample_ids in corrections_dict_raw['NK Gating']['y1=5.0']:
        ylim_1 =  ylim_1 * 5.0
    elif sample_ids in corrections_dict_raw['NK Gating']['y1=6.0']:
        ylim_1 =  ylim_1 *6.0
    elif sample_ids in corrections_dict_raw['NK Gating']['y1=10.0']:
        ylim_1 =  ylim_1 *10 
    else:
        ylim_1 =  ylim_1
    ...
    if sample_ids in corrections_dict_raw['NK Gating']['CUSTOM_sigma+4']:
        ...
        sigma_n = 3.5
    elif sample_ids in corrections_dict_raw['NK Gating']['sigma_4.5']:
        sigma_n = 4.5
    elif sample_ids in corrections_dict_raw['NK Gating']['sigma_5.0']:
        sigma_n = 5
    elif sample_ids in corrections_dict_raw['NK Gating']['sigma_10.0']:
        sigma_n = 10
    else:
        sigma_n = 3
    ...
    if sample_ids in corrections_dict_raw['NK Gating']['CUSTOM_x=1000']:
        ...
        hixlim = 10000
    elif sample_ids in corrections_dict_raw['NK Gating']['hixlim_step+500']:
        hixlim = hixlim +500
    elif sample_ids in corrections_dict_raw['NK Gating']['hixlim_step+1000']:
        hixlim = hixlim+1000
    else:
        hixlim = hixlim 
    
    # CD56+CD16+ NK
    doublePosNK = ag.gateCorner(fcs, "CD56posCD16posNK", parentGate=CD14neg, xCol=CD16, yCol=CD56, xThresh=hixlim, yThresh=ylim_1, 
        yOrientation='upper', xOrientation='upper', scale='bilog',T=100, filePlot=None)
    ...
    # CD56-CD16+ NK
    ############################################ the mian part to be modified #####################################################
    fileName1="/home/wenxiaren/AGcorrector_B_panel/tmp"+date+"-"+plate+"-"+sampleName+"-CD56negCD16posNK.png"
    CD56negCD16posNK = ag.gateCorner(fcs, "CD56negCD16posNK", parentGate=CD14neg, xCol=CD16, yCol=CD56, xThresh=xThresh2, yThresh=ylim_1, yOrientation='lower', 
        xOrientation='upper', scale='bilog',T=100, filePlot=None)
    ...
    CD56negCD16posNK=ag.gatePC(fcs,xCol=CD16, yCol=CD56,name="CD56negCD16posNK",center=center, adjustAngle=adjustAngle_2, widthScale=widthScale_2, heightScale=heightScale_2,parentGate=CD56low,filePlot=None)

    ag.backGate(fcs,population=CD56negCD16posNK,background_population=CD14neg,xCol=CD16, yCol=CD56,markersize=0.5, filePlot=fileName1,scale='bilog', T=100)
...
with st.sidebar:
    ...
    xThresh2 = st.number_input("xThresh2", min_value=0, max_value=10000000, value=10000)
    Thresh2 = st.number_input("Thresh2", min_value=-1000, max_value=10000, value=-100)
    center = st.selectbox("center",options=["density", "centroid"])
    adjustAngle_2 = st.number_input("adjustAngle_2", min_value=-50, max_value=100, value=1)
    widthScale_2 = st.number_input("widthScale_2", min_value=0, max_value=100, value=8)
    heightScale_2= st.number_input("heightScale_2", min_value=0, max_value=100, value=4)
...
for col in row2:
    ...
    unsafe_allow_html=True
                    ...
                    for img in Image_folder:
                        ...
                        img_id = re.sub(r'-CD56negCD16posNK.png','',imd_name)
            ...
            if Update:
                if 'fcs' in st.session_state and st.session_state['fcs'] is not None:
                    ...
                    fig_path= gateGeneralDataSet(
                        fcs, 
                        final_params['xThresh2'],
                        final_params['Thresh2'],
                        final_params['center'],
                        final_params['adjustAngle_2'],
                        final_params['widthScale_2'],
                        final_params['heightScale_2']
                    )
            ...
            if Confirm:
                if not selected_parameters:
                    ...
                    para_dict = {}
                    if "xThresh2" in selected_parameters:
                        para_dict[f"xThresh2 {xThresh2}"] = xThresh2
                    if "Thresh2" in selected_parameters:
                        para_dict[f"Thresh2 {Thresh2}"] = Thresh2
                    if "center'" in selected_parameters:
                        para_dict[f"center {center}"] = center
                    if "adjustAngle_2" in selected_parameters:
                        para_dict[f"adjustAngle_2 {adjustAngle_2}"] = adjustAngle_2
                    if "widthScale_2" in selected_parameters:
                        para_dict[f"widthScale_2 {widthScale_2}"] = widthScale_2
                    if "heightScale_2" in selected_parameters:
                        para_dict[f"heightScale_2 {heightScale_2}"] = heightScale_2

                    YAML_PATH = "/home/wenxiaren/AGcorrector_B_panel/tmp/CD16+CD56-NK_correction.yaml"
