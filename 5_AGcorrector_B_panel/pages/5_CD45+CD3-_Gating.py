...
#!/usr/bin/env python3
"""
Title:5_CD45+CD3-_Gating.py
Date: 2025-05-27
Author: Wenxia Ren
Description: 
Customed Gating for CD45+CD3- cells
"""
...
def gateGeneralDataSet(fcs, xlim_step,ylim_step):
    SSC="SSC 488/10-A"
    FSC="FSC 488/10-A"
    IgA="FITC IgA-A"
    ...
    fileName="/home/wenxiaren/AGcorrector_B_panel/tmp"+date+"-"+plate+"-"+sampleName+"-CD45pos.png"
    ylim1=ylim+ylim_step
    xlim1=xlim+xlim_step
...
with st.sidebar:
    ...
    xlim_step = st.number_input("xlim_step", min_value=-10000, max_value=10000, value=0)
    ...
    unsafe_allow_html=True
            ...
            if Update:
                if 'fcs' in st.session_state and st.session_state['fcs'] is not None:
                    ...
                    fig_path = gateGeneralDataSet(fcs, xlim_step,ylim_step)
                    cols = st.columns(4)
                    with cols[0]:
            ...
            if Confirm:
                if 'current_id' in st.session_state:
                    ...
                    para_dict = {
                        f"xlim_step { xlim_step}": xlim_step,
                        f"ylim_step {ylim_step}": ylim_step
                    }
                YAML_PATH = "/home/wenxiaren/AGcorrector_B_panel/tmp/CD45+CD3-_correction.yaml"
