...
#!/usr/bin/env python3
"""
Title:3_PBMC_Gating.py
Date: 2025-05-16
Author: Wenxia Ren
Description:
This program is used for customed gating of PBMC population in the B cell panel in ImmSight project.
The user can adjust the parameters in the sidebar to get the optimal gating result for each sample.
The parameters include:
1) direction: the direction of the horizontal path gating, can be "both" or "up"
2) xboundaries: the x boundaries for the horizontal path gating, can be a tuple of two values
3) no_clutter_thresh: the threshold for the no_clutter gating
4) cut_off_xlim: the x limit for the final PBMC gating
5) cut_off_ylim: the y limit for the final PBMC gating
Note:
1) The input files include the ID file, the PATH file and the PNG file. The ID file contains the sample IDs, the PATH file contains the absolute paths for all the fcs files, and the PNG file contains the original images for each sample.
2) The output images are saved in the tmp folder in the AGcorrector_T_panel directory.  
"""
...
def PBMC_Gating(fcs, direction, xboundaries,no_clutter_thresh, cut_off_xlim, cut_off_ylim):
    ...
    fileName_2 ="/home/wenxiaren/AGcorrector_B_panel/tmp"+date+"-"+plate+"-"+sampleName+"- xlim.png"
    ...
    fileName_3 ="/home/wenxiaren/AGcorrector_B_panel/tmp"+date+"-"+plate+"-"+sampleName+"- ylim.png"
    ...
    fileName_4="/home/wenxiaren/AGcorrector_B_panel/tmp"+date+"-"+plate+"-"+sampleName+"- PBMC.png"
    ...
    fileName_1 ="/home/wenxiaren/AGcorrector_B_panel/tmp"+date+"-"+plate+"-"+sampleName+"- PBMC_back.png"
...
with st.sidebar:
    ...
    xboundaries =  st.slider("Select a range", min_value=0, max_value=214000, value=(214000, 214000), step=1000)
    ...
    cut_off_ylim = st.number_input("cut_off_ylim", min_value=0, max_value=250000, value=120000, step=500)
    Another_Cell_Distribution = st.checkbox("Dijkstra failure",value=False)
    ...
    unsafe_allow_html=True
            ...
            if st.session_state.get('update_triggered', False):
                ...
                if 'fcs' in st.session_state and st.session_state['fcs'] is not None:
                    ...
                    fileName_1, fileName_4, fileName_3,fileName_2 = PBMC_Gating(fcs, direction,xboundaries,no_clutter_thresh, cut_off_xlim, cut_off_ylim)
            ...
            if Confirm:
                if 'current_id' in st.session_state:
                    ...
                    para_dict = {
                        f"No_Clutter_{no_clutter_thresh}": no_clutter_thresh,
                        f"xlim_{cut_off_xlim}": cut_off_xlim,
                        f"Ylim_{cut_off_ylim}": cut_off_ylim,
                        f"Dijkstra failure":Another_Cell_Distribution,
                    }
                YAML_PATH = "/home/wenxiaren/AGcorrector_B_panel/tmp/PBMC_correction.yaml"
                if os.path.exists(YAML_PATH):
                    with open(YAML_PATH,"r") as in1:
                        correction_yaml = yaml.safe_load(in1) or {}
                else:
                    correction_yaml = {}
                # Update combo_key entry
