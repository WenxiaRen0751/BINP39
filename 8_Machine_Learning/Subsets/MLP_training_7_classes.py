...
Date: 2025-10-11
Description:
This script is to train an MLP model for 7 subsets classification of flow cytometry data.
...
def MLP_training(X, y, features,out_path="mlp.pt", epochs=25):

    """
    Train a 3-layer MLP model for 7-class flow cytometry classification.
    """
    ...
    leaf_to_name = {
        ...
        2: "singlet/CD45posCD3neg/CD19neg/CD14pos/Inter_Mono",
        3: "singlet/CD45posCD3neg/CD19neg/CD14pos/non_CM",
        4: "singlet/CD45posCD3neg/CD19neg/CD14neg",
        5: "singlet/CD45posCD3pos",
        6: "singlet/rest_of_singlets"
    }
...
# Load data 
train_data_path = "/home/wenxia/Part_3_ML/Dataset/train_dataset_classes.pt"
data = torch.load(train_data_path, map_location="cpu")
X = data["X"].cpu().numpy()  
y = data["y"].cpu().numpy()   
feature_indices = [features.index(feature) for feature in expected_features]
X = X[:, feature_indices] 
...
X = np.concatenate([X,CD14_by_16], axis=1)
# final feature
...
# Train
MLP_training(X, y,expected_features,out_path="mlp_7classes.pt", epochs=25)
