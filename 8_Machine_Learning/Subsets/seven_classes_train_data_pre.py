...
Date: 2025-10-01
Description: 
This script is to extract the train data. The classification of 7 classes
    # 0: CD19pos，1: CM, 2: Inter_Mono, 3: non_CM, 4: CD14neg, 5: CD45posCD3pos, 6: rest of singlets generated from the finalized gating strategy, to train the MLP model.
"""
...
def gateGeneralDataSet(fcs, *args, **kwargs):
    ...
    rest_of_singlets = list(set(singlets) - set(CD45pos) - set(CD45posCD3pos))# label as 6
    ...
    logtrans_dict = {"BV786 CD14-A":"BV786 CD14-A_v2","BV605 CD19-A":"BV605 CD19-A_v2","APC CD16-A":"APC CD16-A_v2","APC-H7 CD45-A":"APC-H7 CD45-A_v2","Alexa 700 CD3-A":"Alexa 700 CD3-A_v2"}
    ...
    # downsampling the
    df_CD19pos = df_0.loc[CD19pos].sample(frac=0.2)
    df_CD19pos["label"] = 0

    df_CM = df_0.loc[CM].sample(frac=0.08)
    df_CM["label"] = 1

    df_Inter_Mono = df_0.loc[Inter_Mono].sample(frac=0.6)
    df_Inter_Mono["label"] = 2

    df_non_CM = df_0.loc[non_CM].sample(frac=1)
    df_non_CM["label"] = 3

    df_CD14neg = df_0.loc[CD14neg].sample(frac=0.08)
    df_CD14neg["label"] = 4
    
    df_CD45posCD3pos= df_0.loc[CD45posCD3pos].sample(frac=0.02)
    df_CD45posCD3pos["label"] = 5

    df_rest_of_singlets = df_0.loc[rest_of_singlets].sample(frac=0.2)
    df_rest_of_singlets["label"] = 6
    # combining the downsampled
    df = pd.concat([df_CD19pos,df_CM,df_Inter_Mono,df_non_CM,df_CD14neg,df_CD45posCD3pos,df_rest_of_singlets])
    #df = pd.concat([df_CD19pos,df_CM,df_Inter_Mono,df_non_CM,df_CD14neg])
    # combining the downsampled
...
def traindata_extract(paths, out_pt, out_id_csv):
    ...
    """
    Process a list of FCS file paths, gate 7 populations, extract features and labels,
    and save the combined dataset as a PyTorch tensor file along with an index CSV.
    1. For each FCS file:
       - Load the FCS file using AliGater.
       - Apply gating strategy to identify 7 populations.
...
if __name__ == '__main__':
    ...
    traindata_extract(train_paths,
        "/home/wenxiaren/Part_3_ML/Dataset/Multiple_Classes_CD3neg_monocytes/train_data/train_dataset_classes.pt",
        "/home/wenxiaren/Part_3_ML/Dataset/Multiple_Classes_CD3neg_monocytes/train_data/train_index.csv"
    )
