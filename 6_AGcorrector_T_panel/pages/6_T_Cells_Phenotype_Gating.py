...
#!/usr/bin/env python3
"""
Title: 6_T_Cells_Phenotype_Gating.py
Date: 2025-05-27
Author: Wenxia Ren
Description: 
This script defines the gating strategy for T cell phenotype analysis 
"""
...
# set the path for the corrections YAML file, including all the manual adjustments
...
def gateGeneralDataSet(fcs, CD45RA_interval, CCR7_lim_down_sigma, CCR7_lim_upper_sigma,CD8_ylim,Top_thresh,CD8_xlim):
    SSC = "SSC 488/10-A"
    FSC = "FSC 488/10-A"
    FSC_H = "FSC 488/10-H"
    CD39 = "BB515 CD39-A"
    ...
    (prob_theoretical, ordered_data), (slope, intercept, r) = stats.probplot(tempCD4quad_top(), dist="norm")
    if maxVal > median:
    ...
    fileName_2="/home/wenxiaren/AGcorrector_T_panel/tmp"+Image_ID_prefix+"-CD4pos-phenotype.png"
    ...
    fcs.update(ag.AGgate(effector_memory_CD4pos, CD4pos, xCol=CCR7, yCol=CD45RA, name="effector_memory_CD4pos"),
     ...
     # the phenotype of CD8pos
    ylim = ag.valleySeek(fcs, xCol=CD45RA, interval=CD8_ylim, require_local_min=True, parentGate=CD8pos, scale='bilog', T=200)
    if ylim == ag.np.inf:
        ylim=200
    if ylim < 200:
        ylim=200
    tempCD8quad_top = ag.gateThreshold(fcs,xCol=CCR7, yCol=CD45RA,name="topTmp",parentGate=CD8pos, 
                                    thresh =Top_thresh, population='upper',orientation='horizontal',scale='bilog', T=200,update=False)
    xlim = ag.valleySeek(fcs,xCol=CCR7,interval=CD8_xlim,require_local_min=True, parentGate=tempCD8quad_top,scale='bilog', T=1000)
    if xlim == ag.np.inf:
        xlim=600
    fileName_3="/home/wenxiaren/AGcorrector_T_panel/tmp"+Image_ID_prefix+"-CD8pos-phenotype.png"
    effector_CD8pos, naive_CD8pos, central_memory_CD8pos, effector_memory_CD8pos = ag.quadGate(fcs=fcs, 
                                                                                            names=['effector_CD8pos', 'naive_CD8pos', 'central_memory_CD8pos', 'effector_memory_CD8pos'], 
                                                                                            xCol=CCR7, yCol=CD45RA, xThresh=xlim, yThresh=ylim, parentGate=CD8pos, 
                                                                                            scale='bilog', T=200, filePlot=fileName_3)
    fcs.update(ag.AGgate(effector_CD8pos, CD8pos, xCol=CCR7, yCol=CD45RA, name="effector_CD8pos"), QC=True, MFI=True,
               MFI_type='current', extra_MFI=extra_MFI_list, xlim=[0, 3000], ylim=[-200, 1000],
               scale='bilog', T=200)
    fcs.update(ag.AGgate(naive_CD8pos, CD8pos, xCol=CCR7, yCol=CD45RA, name="naive_CD8pos"), QC=False, MFI=True,
               MFI_type='current', extra_MFI=extra_MFI_list)
    fcs.update(ag.AGgate(central_memory_CD8pos, CD8pos, xCol=CCR7, yCol=CD45RA, name="central_memory_CD8pos"), QC=False,
               MFI=True, MFI_type='current', extra_MFI=extra_MFI_list)
    fcs.update(ag.AGgate(effector_memory_CD8pos, CD8pos, xCol=CCR7, yCol=CD45RA, name="effector_memory_CD8pos"),
               QC=False, MFI=True, MFI_type='current', extra_MFI=extra_MFI_list)
    
    return fileName_2, r, mean, maxVal, fileName_3
...
with st.sidebar:  
    ...
    CD45RA_interval = st.text_input("CD45RA_interval (default: 50,400)", value="50,400", 
                            help="Enter two numbers separated by comma, e.g., 50,400")
    try:
        # Parse the input string to create the list
        CD45RA_values = [int(x.strip()) for x in CD45RA_interval.split(',')]
        if len(CD45RA_values) == 2:
            CD45RA_interval = CD45RA_values
            st.info(f"CD45RA_interval: {CD45RA_interval}")
        else:
            st.error("❌ Please enter exactly two values separated by comma")
            CD45RA_interval = [50, 400]  # fallback to default
    except ValueError:
        st.error("❌ Please enter valid numbers separated by comma")
        CD45RA_interval = [50, 400]  # fallback to default
    CD45RA_interval= list(CD45RA_interval)
    CCR7_lim_down_sigma= st.number_input("CCR7_lim_down_sigma", min_value=-10.0, max_value=10.0,step=1.,format="%.1f")
    CCR7_lim_upper_sigma= st.number_input("CCR7_lim_upper_sigma", min_value=-10.0, max_value=10.0,step=1.,format="%.1f")
    CD8_ylim = st.text_input("CD8_ylim,the default is 50-400", value="50,400", 
                            help="Enter two numbers separated by comma, e.g., 50,400")
    try:
        # Parse the input string to create the list
        CD8_ylim = [int(x.strip()) for x in CD8_ylim.split(',')]
        if len(CD8_ylim) == 2:
            CD8_ylim = CD8_ylim
            st.info(f"CD8_ylim: {CD8_ylim}")
        else:
            st.error("❌ Please enter exactly two values separated by comma")
            CD8_ylim = [50, 400]  # fallback to default
    except ValueError:
        st.error("❌ Please enter valid numbers separated by comma")
        CD8_ylim = [50, 400]  # fallback to default
    CD8_ylim= list(CD8_ylim)
    Top_thresh= st.number_input("Top_thresh, the default is 500", min_value=100, max_value=10000,value=500)
    CD8_xlim =  st.slider("CD8_xlim,the default is 100-800", min_value=-200, max_value=1000, value=(100, 800), step=50)
    CD8_xlim= list(CD8_xlim)
    st.markdown("---")
...
for col in row2:
    ...
    unsafe_allow_html=True
                    ...
                    if matched_images:
                        # Separate CD4 and CD8 images
                        cd4_images = []
                        cd8_images = []
                        
                        for img in matched_images:
                            img_name = img.name
                            if img_name.endswith("-CD4pos-N_CM_E_EM.png"):
                                cd4_images.append(img)
                            elif img_name.endswith("-CD8pos_N_CM_E_EM.png"):
                                cd8_images.append(img)
                        
                        # Create columns for display
                        cols = st.columns(4)
                        
                        # Display CD4 images on the left
                        with cols[0]:
                            st.subheader("CD4+ T cells")
                            for img in cd4_images:
                                st.image(Image.open(img), use_container_width=True)
                        
                        # Display CD8 images on the right
                        with cols[1]:
                            st.subheader("CD8+ T cells")
                            for img in cd8_images:
                                st.image(Image.open(img), use_container_width=True)
                                
                        # Optional: Display warning if expected images are missing
                        if not cd4_images:
                            st.warning("⚠️ No CD4+ phenotype images found")
                        if not cd8_images:
                            st.warning("⚠️ No CD8+ phenotype images found")
                    else:
            ...
            if Update:
                if not selected_parameters:
                    ...
                    default_values = {
                        ...
                        'CCR7_lim_down_sigma': 0.0,
                        'CCR7_lim_upper_sigma': 0.0,
                        'CD8_ylim': [50, 400],
                        'Top_thresh': 500,
                        'CD8_xlim': [100, 800]
                    }
                    ...
                    fileName_2, r, mean, maxVal, fileName_3 = gateGeneralDataSet(
                        fcs, 
                        final_params['CD45RA_interval'],
                        final_params['CCR7_lim_down_sigma'],
                        final_params['CCR7_lim_upper_sigma'],
                        final_params['CD8_ylim'],
                        final_params['Top_thresh'],
                        final_params['CD8_xlim']
                    )
                    st.write(f"r: {r}") 
                    st.write(f"mean: {mean}") 
                    st.write(f"maxVal: {maxVal}") 
                    cols = st.columns(4)
                    with cols[0]:
            ...
            if Confirm:
                if not selected_parameters:
                    ...
                    para_dict = {}
                    if "CD45RA_interval" in selected_parameters:
                        para_dict[f"CD45RA_interval {CD45RA_interval}"] = CD45RA_interval
                    if "CCR7_lim_down_sigma" in selected_parameters:
                        para_dict[f"CCR7_lim_down_sigma {CCR7_lim_down_sigma}"] = CCR7_lim_down_sigma
                    if "CCR7_lim_upper_sigma" in selected_parameters:
                        para_dict[f"CCR7_lim_upper_sigma {CCR7_lim_upper_sigma}"] = CCR7_lim_upper_sigma
                    if "CD8_ylim" in selected_parameters:
                        para_dict[f"CD8_ylim {CD8_ylim}"] = CD8_ylim
                    if "Top_thresh" in selected_parameters:
                        para_dict[f"Top_thresh {Top_thresh}"] = Top_thresh
                    if "CD8_xlim" in selected_parameters:
                        para_dict[f"CD8_xlim {CD8_xlim}"] = CD8_xlim
                    
                    YAML_PATH = "/home/wenxiaren/AGcorrector_T_panel/tmp/phenotype_correction.yaml"
