...
#!/usr/bin/env python3
"""
Title:5_T_Cells_Subsets_Gating.py
Date: 2025-05-27
Author: Wenxia Ren
Description: 
Customed Gating for T cells subsets 
"""
...
def gateGeneralDataSet(fcs, CD4_interval,CD8_interval):
    SSC = "SSC 488/10-A"
    FSC = "FSC 488/10-A"
    FSC_H = "FSC 488/10-H"
    CD39 = "BB515 CD39-A"
    ...
    ##################################################### the main gating for CD4 and CD8 subsets #####################################################
    # Next: 4)  CD4/CD8 gating
    # CD4/CD8
    interval_CD4 = CD4_interval
    interval_CD8 = CD8_interval
    xlim = ag.valleySeek(fcs, xCol=CD8, parentGate=CD3pos, interval=interval_CD4, require_local_min=True, scale='bilog',T=200)
    if xlim == ag.np.inf:
        xlim = 1500
    ...
    ylim = ag.valleySeek(fcs, xCol=CD4, parentGate=CD8neg_tmp, interval=interval_CD8, require_local_min=True,
                        scale='bilog', T=200)
    if ylim == ag.np.inf:
        ylim = 200
    if ylim < 100: 
        ylim = 200  
    fileName_1="/home/wenxiaren/AGcorrector_T_panel/tmp"+Image_ID_prefix+"-CD4CD8.png"
    CD4pos, CD4CD8_doublepos, CD8pos, CD4CD8_doubleneg = ag.quadGate(fcs, names=["CD4pos", "CD4posCD8pos", "CD8pos",
                                                                    ...
                                                                    scale='bilog', T=200, filePlot=fileName_1)
...
with st.sidebar:
    ...
    CD4_interval =  st.slider("CD4_interval", min_value=0, max_value=30000, value=(0, 10000), step=100)
    CD4_interval = list(CD4_interval)
    CD8_interval =  st.slider("CD8_interval", min_value=0, max_value=5000, value=(50, 1000), step=50)
    CD8_interval = list(CD8_interval)
    ...
    unsafe_allow_html=True
            ...
            if Update:
                if 'fcs' in st.session_state and st.session_state['fcs'] is not None:
                    ...
                    fig_path = gateGeneralDataSet(fcs, CD4_interval, CD8_interval)     
                    cols = st.columns(4)
                    with cols[0]:
            ...
            if Confirm:
                if 'current_id' in st.session_state:
                    ...
                    para_dict = {
                        f"CD4_interval {CD4_interval}": CD4_interval,
                        f"CD8_interval {CD8_interval}": CD8_interval,
                    }
                YAML_PATH = "/home/wenxiaren/AGcorrector_T_panel/tmp/CD4_8_correction.yaml"
