...
#!/usr/bin/env python3
"""
Title:11_Treg_subpopulation.py
Date: 2025-05-27
Author: Wenxia Ren
Description: 
Gating Treg and CD194+Treg subpopulations
...
def gateGeneralDataSet(fcs, Treg_subpo_xlim,Treg_subpo_ylim,CD194pos_Treg_subpo_xlim,CD194pos_Treg_subpo_ylim):
    SSC = "SSC 488/10-A"
    FSC = "FSC 488/10-A"
    FSC_H = "FSC 488/10-H"
    CD39 = "BB515 CD39-A"
    ...
    xmaxval_step = 0
    pattern_1 = r'-?\d+\.?\d*$'
    xmaxval_step = para_extracter('xparam_step',pattern_1)
    ...
    ylim_step = 0
    pattern_2 = r'-?\d+\.?\d*$'
    ylim_step = para_extracter('ylim_step',pattern_2)
    ylim = ylim+ylim_step
    ...
    ymaxval_step=0
    pattern_3 = r'-?\d+\.?\d*$'
    ymaxval_step = para_extracter('ymaxval_step',pattern_3)
    ...
    xlim_step=0
    pattern_4 = r'-?\d+\.?\d*$'
    xlim_step = para_extracter('xlim_step',pattern_4)
    xlim = xlim + xlim_step
    if ymaxVal < 0:
        ymaxVal = 0
    ...
    xparam_step = 0
    pattern_5 = r'-?\d+\.?\d*$'
    xparam_step = para_extracter('xparam_step',pattern_5)
    bezierXParam[0] = bezierXParam[0]+xparam_step

    yparam_step = 0
    pattern_6 = r'-?\d+\.?\d*$'
    yparam_step = para_extracter('yparam_step',pattern_6)
    ...
    Tregs = ag.gateBezier(fcs, name="Tregs", xCol=CD25, yCol=CD127, endExtension='right', startExtension='down',
                          ...
                          xParam=bezierXParam, yParam=bezierYParam, scale='bilog', T=200, filePlot=None)
    ...
    ##################################################### the main part Treg_subpopulation ###########################################

    ylim = ag.valleySeek(fcs, xCol=CD45RA, parentGate=Tregs, interval=[0,1000], scale='bilog', T=200)
    tmpbottomTreg = ag.gateThreshold(fcs, xCol=CD25, yCol=CD45RA, name="bottomTreg", parentGate=Tregs, thresh=ylim,
    ...
    if xlim < 1000:
        xlim = 1000
    xlim = xlim + Treg_subpo_xlim
    fileName_1 = "/home/wenxiaren/AGcorrector_T_panel/tmp/"+Image_ID_prefix+"-activatedTreg.png"
    activatedTreg = ag.gateCorner(fcs, name="activatedTreg", xCol=CD25, yCol=CD45RA, xThresh=xlim, yThresh=ylim,
                                  ...
                                  filePlot=fileName_1)
    fileName_2 ="/home/wenxiaren/AGcorrector_T_panel/tmp/"+Image_ID_prefix+"-secretingTreg.png"
    ...
    fcs.update(ag.AGgate(activatedTreg, Tregs, xCol=CD25, yCol=CD45RA, name="activatedTreg"), QC=True, MFI=True,
               ...
               T=200)
    fcs.update(ag.AGgate(secretingTreg, Tregs, xCol=CD25, yCol=CD45RA, name="secretingTreg"), QC=False, MFI=True,
               MFI_type='current', extra_MFI=extra_MFI_list)
    fcs.update(ag.AGgate(restingTreg, Tregs, xCol=CD25, yCol=CD45RA, name="restingTreg"), QC=False, MFI=True,
               MFI_type='current', extra_MFI=extra_MFI_list)

# Treg_subpo_xlim,Treg_subpo_ylim,CD194pos_Treg_subpo_xlim,CD194pos_Treg_subpo_ylim
    ...
    for keys, ids in corrections_dict_raw.items():
        if keys.startswith('CD194_Tregs_interval'):
            ...
            if CD194_Tregs_match and ID in ids:
                ...
                break
    xlim = ag.valleySeek(fcs, xCol=CD194, parentGate=Tregs, interval=CD194_Tregs_interval, require_local_min=True, sigma=3,
                         scale='bilog', T=200)
    if xlim == ag.np.inf:
        xlim = 1500
    CD194_Tregs = ag.gateThreshold(fcs=fcs, name="CD194pos", xCol=CD194, yCol=CD127, parentGate=Tregs, thresh=xlim,
                                   orientation='vertical', scale='bilog', T=200, population='upper', filePlot=None)
    fcs.update(ag.AGgate(CD194_Tregs, Tregs, xCol=CD194, yCol=CD127, name="CD194pos"), QC=True, MFI=True,
               MFI_type="current", extra_MFI=extra_MFI_list, xlim=[0, 25000], ylim=[0, 700],
               scale='bilog', T=200)

    ylim = ag.valleySeek(fcs, xCol=CD45RA, parentGate=Tregs, interval=[0, 1000], scale='bilog', T=200)
    tmpbottomTreg = ag.gateThreshold(fcs, xCol=CD25, yCol=CD45RA, name="bottomTreg", parentGate=CD194_Tregs,
                                     thresh=ylim, population='lower', orientation='horizontal', scale='bilog', T=200,
                                     update=False)
    ...
    if ylim < 100:
        ylim = 100
    ylim = ylim + CD194pos_Treg_subpo_ylim
    # We use this to gate the activated Tregs (Dont save image tho, unneccesary)
    restingCD194posTreg = ag.gateThreshold(fcs, xCol=CD25, yCol=CD45RA, name="restingCD194posTreg", thresh=ylim,
                                           parentGate=CD194_Tregs, scale='bilog', T=200, population='upper',
                                           orientation='horizontal', update=False, filePlot=None)

    # P1 version of secreting/activated Treg
    bottomTreg = ag.gateThreshold(fcs, xCol=CD25, yCol=CD45RA, name="tmp_bottom_CD194postreg", thresh=ylim,
                                  parentGate=CD194_Tregs, scale='bilog', T=200, population='lower',
                                  orientation='horizontal', update=False, filePlot=None)
    ...
    if xlim < 1000:
        xlim = 1000
    xlim = xlim + CD194pos_Treg_subpo_xlim
    fileName_3 = "/home/wenxiaren/AGcorrector_T_panel/tmp/"+Image_ID_prefix+"-activatedCD194posTreg.png"
    activatedCD194posTreg = ag.gateCorner(fcs, name="activatedCD194posTreg", xCol=CD25, yCol=CD45RA, xThresh=xlim,
                                          yThresh=ylim, xOrientation="upper", yOrientation="lower",
                                          parentGate=CD194_Tregs, scale='bilog', T=200, filePlot=fileName_3)
    fileName_4 = "/home/wenxiaren/AGcorrector_T_panel/tmp/"+Image_ID_prefix+"-secretingCD194posTreg.png"
    secretingCD194posTreg = ag.gateCorner(fcs, name="secretingCD194posTreg", xCol=CD25, yCol=CD45RA, xThresh=xlim,
                                          yThresh=ylim, xOrientation="lower", yOrientation="lower",
                                          parentGate=CD194_Tregs, scale='bilog', T=200, filePlot=fileName_4)

    # Update calls for CD194postregs
    fcs.update(ag.AGgate(activatedCD194posTreg, CD194_Tregs, xCol=CD25, yCol=CD45RA, name="activatedCD194posTreg"),
               QC=True, MFI=True, MFI_type='current', extra_MFI=extra_MFI_list, xlim=[0, 3000],
               ylim=[0, 1000],
               scale='bilog', T=200)
    fcs.update(ag.AGgate(secretingCD194posTreg, CD194_Tregs, xCol=CD25, yCol=CD45RA, name="secretingCD194posTreg"),
               QC=False, MFI=True, MFI_type='current', extra_MFI=extra_MFI_list)
    fcs.update(ag.AGgate(restingCD194posTreg, CD194_Tregs, xCol=CD25, yCol=CD45RA, name="restingCD194posTreg"),
               QC=False, MFI=True, MFI_type='current', extra_MFI=extra_MFI_list)
...
with st.sidebar:  
    ...
    Treg_subpo_xlim = st.number_input("Treg_subpo_xlim",min_value=-100000.0, max_value=100000.0,step=1.,format="%.1f")
    Treg_subpo_ylim = st.number_input("Treg_subpo_ylim",min_value=-100000.0, max_value=100000.0,step=1.,format="%.1f")
    CD194pos_Treg_subpo_xlim = st.number_input("CD194pos_Treg_subpo_xlim",min_value=-100000.0, max_value=100000.0,step=1.,format="%.1f")
    CD194pos_Treg_subpo_ylim = st.number_input("CD194pos_Treg_subpo_ylim",min_value=-100000.0, max_value=100000.0,step=1.,format="%.1f")
    st.markdown("---")
    ...
    parameter_options = [
        "Treg_subpo_xlim",
        "Treg_subpo_ylim",
        "CD194pos_Treg_subpo_xlim",
        "CD194pos_Treg_subpo_ylim"
    ]
            ...
            if Update:
                if not selected_parameters:
                    ...
                    default_values = {
                        "Treg_subpo_xlim":0,
                        "Treg_subpo_ylim":0,
                        "CD194pos_Treg_subpo_xlim":0,
                        "CD194pos_Treg_subpo_ylim":0
                    }
                    ...
                    fileName_1, fileName_2, fileName_3, fileName_4  = gateGeneralDataSet(
                        fcs, 
                        final_params['Treg_subpo_xlim'],
                        final_params['Treg_subpo_ylim'],
                        final_params['CD194pos_Treg_subpo_xlim'],
                        final_params['CD194pos_Treg_subpo_ylim']
                    )
                    cols = st.columns(4)
                    with cols[0]:
                        st.image(fileName_1, use_container_width=True)
                    with cols[1]:
                        st.image(fileName_2, use_container_width=True)
                    with cols[2]:
                        st.image(fileName_3, use_container_width=True)
                    with cols[3]:
                        st.image(fileName_4, use_container_width=True)
            if Confirm:
                if not selected_parameters:
                    ...
                    para_dict = {}
                    if "Treg_subpo_xlim" in selected_parameters:
                        para_dict[f"Treg_subpo_xlim {Treg_subpo_xlim}"] = Treg_subpo_xlim
                    if "Treg_subpo_ylim" in selected_parameters:
                        para_dict[f"Treg_subpo_ylim {Treg_subpo_ylim}"] = Treg_subpo_ylim
                    if "CD194pos_Treg_subpo_xlim" in selected_parameters:
                        para_dict[f"CD194pos_Treg_subpo_xlim {CD194pos_Treg_subpo_xlim}"] = CD194pos_Treg_subpo_xlim
                    if "CD194pos_Treg_subpo_ylim" in selected_parameters:
                        para_dict[f"CD194pos_Treg_subpo_ylim {CD194pos_Treg_subpo_ylim}"] = CD194pos_Treg_subpo_ylim
                  
                    YAML_PATH = "/home/wenxiaren/AGcorrector_T_panel/tmp/Treg_subpopulation_correction.yaml"
