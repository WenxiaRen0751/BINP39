...
"""
Title:12_Th_Gating.py
Date: 2025-05-27
Author: Wenxia Ren
Description:
Gating Th1, Th2, Th17 from non-naive CD4+ T cells
"""
...
def gateGeneralDataSet(fcs, Th_CCR6_interval,CD194_sigma_1,CD194_sigma_2):
    SSC = "SSC 488/10-A"
    FSC = "FSC 488/10-A"
    FSC_H = "FSC 488/10-H"
    CD39 = "BB515 CD39-A"
    ...
    ########################################### THE MAIN PART FOR Th1, Th2, Th17 GATING #############################################
    # gating the internediate Th1, Th2, Th17
    non_naive_T = ag.AGgate(effector_CD4pos() + central_memory_CD4pos() + effector_memory_CD4pos(), CD4pos, CCR7, CD45RA, name="non_naive_T")
    fcs.update(ag.AGgate(non_naive_T, CD4pos, xCol=CCR6, yCol=CD45RA, name="non_naive_T"), QC=False, MFI=True, MFI_type='current', extra_MFI=extra_MFI_list)
    ...
    if xlim < 300:
        ...
        #### here is the first argument from the function
    ...
    fileName_1="/home/wenxiaren/AGcorrector_T_panel/tmp/"+Image_ID_prefix+"-Th_1_2_17.png"
    fig_1,fig_2,fig_3,fig_4 = ag.quadGate(fcs=fcs, names=['fig_1', 'fig_2', 'fig_3', 'fig4'], xCol=CCR6, yCol=CD194, xThresh=xlim, yThresh=ylim, parentGate=non_naive_T, scale='bilog', T=1000, filePlot=fileName_1)

    return fileName_1, xlim,ylim
...
with st.sidebar:  
    ...
    Th_CCR6_interval = st.text_input("Th_CCR6_interval (default: 50,400)", value="50,400", 
                            help="Enter two numbers separated by comma, e.g., 50,400")
    try:
        # Parse the input string to create the list
        Th_CCR6 = [int(x.strip()) for x in Th_CCR6_interval.split(',')]
        if len(Th_CCR6) == 2:
            Th_CCR6_interval = Th_CCR6 
            st.info(f"Th_CCR6_interval: {Th_CCR6_interval}")
        else:
            st.error("❌ Please enter exactly two values separated by comma")
            Th_CCR6_interval = [800,5000]  # fallback to default
    except ValueError:
        st.error("❌ Please enter valid numbers separated by comma")
        Th_CCR6_interval = [800,5000] # fallback to default
    Th_CCR6_interval= list(Th_CCR6_interval)

    CD194_sigma_1= st.number_input("CD194_sigma_1", min_value=-10.0, max_value=10.0,step=1.,format="%.1f")
    CD194_sigma_2= st.number_input("CD194_sigma_2", min_value=-10.0, max_value=10.0,step=1.,format="%.1f")
    st.markdown("---")
    ...
    unsafe_allow_html=True
            ...
            if Update:
                if not selected_parameters:
                    ...
                    fileName_1, xlim,ylim = gateGeneralDataSet(
                        fcs, 
                        final_params['Th_CCR6_interval'],
                        final_params['CD194_sigma_1'],
                        final_params['CD194_sigma_2']
                    )
                    st.write(f"CCR6_lim: {xlim}") 
                    st.write(f"CD194_lim: {ylim}") 
                    cols = st.columns(4)
                    with cols[0]:
            ...
            if Confirm:
                if not selected_parameters:
                    ...
                    para_dict = {}
                    if "Th_CCR6_interval" in selected_parameters:
                        para_dict[f"Th_CCR6_interval {Th_CCR6_interval}"] = Th_CCR6_interval
                    if "CD194_sigma_1" in selected_parameters:
                        para_dict[f"CD194_sigma_1 {CD194_sigma_1}"] = CD194_sigma_1
                    if "CD194_sigma_2" in selected_parameters:
                        para_dict[f"CD194_sigma_2 {CD194_sigma_2}"] = CD194_sigma_2
                
                    YAML_PATH = "/home/wenxiaren/AGcorrector_T_panel/tmp/Th_correction.yaml"
