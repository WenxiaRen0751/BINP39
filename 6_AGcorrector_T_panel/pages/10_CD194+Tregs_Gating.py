...
#!/usr/bin/env python3
"""
Title:10_CD194+Tregs_Gating.py
Date: 2025-05-27
Author: Wenxia Ren
Description: 
Customed Gating for CD194+Tregs population in T panel
"""
...
def gateGeneralDataSet(fcs,CD194_Tregs_interval):
    SSC = "SSC 488/10-A"
    FSC = "FSC 488/10-A"
    FSC_H = "FSC 488/10-H"
    CD39 = "BB515 CD39-A"
...
with st.sidebar:  
    ...
    CD194_Tregs_interval = st.text_input("CD194_Tregs_interval (default:500,5000)", value="500,5000", 
                            help="Enter two numbers separated by comma, e.g., 500,5000")
    try:
        # Parse the input string to create the list
        CD194_Tregs_interval = [int(x.strip()) for x in CD194_Tregs_interval.split(',')]
        if len(CD194_Tregs_interval) == 2:
            CD194_Tregs_interval = CD194_Tregs_interval
            st.info(f"CD194_Tregs_interval: {CD194_Tregs_interval}")
        else:
            st.error("❌ Please enter exactly two values separated by comma")
            CD194_Tregs_interval = [500,5000]  # fallback to default
    except ValueError:
        st.error("❌ Please enter valid numbers separated by comma")
        CD194_Tregs_interval = [500,5000] # fallback to default

    CD194_Tregs_interval= list(CD194_Tregs_interval)
    ...
    unsafe_allow_html=True
            ...
            if Update:
                if not selected_parameters:
                    ...
                    fileName_2 = gateGeneralDataSet(
                        fcs, 
                        final_params['CD194_Tregs_interval']
                    )
                    cols = st.columns(2)
                    with cols[0]:
            ...
            if Confirm:
                if not selected_parameters:
                    ...
                    para_dict = {}
                    if "CD194_Tregs_interval" in selected_parameters:
                        para_dict[f"CD194_Tregs_interval {CD194_Tregs_interval}"] = CD194_Tregs_interval
                    
                    YAML_PATH = "/home/wenxiaren/AGcorrector_T_panel/tmp/T_CD194_Treg_correction.yaml"
